...
//Check user's current selected channel [sindh,kpk,rwp,punjab]
if(isset($_COOKIE['channel']))
{
	$channel = "_".strtolower($_COOKIE['channel']);
}
else
{
	$channel = '_sindh';
...
$config['dbTableNames']['online']		= 'ajax_chat_online'.$channel;
$config['dbTableNames']['messages']		= 'ajax_chat_messages'.$channel;
$config['dbTableNames']['bans']			= 'ajax_chat_bans'.$channel;
$config['dbTableNames']['invitations']	= 'ajax_chat_invitations'.$channel;
// echo "<pre>"; 
// print_r($config['dbTableNames']);
// die();
// Available languages:
